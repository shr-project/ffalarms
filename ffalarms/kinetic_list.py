...
# This is slightly modified version of the kinetic list Python Edje
# example by Kenneth Rohde Christiansen
# (http://trac.enlightenment.org/e/browser/trunk/BINDINGS/python/python-edje/examples/evas-demo/03-kinetic_list
# at revision svn 38572)
# 
# Additions:
# - thumbnails are optional
# - support for multiple selection
# - do not consider scrolling if a mouse position moved
#   within a single item (touch screen friendly)
# - allow extra fields in a row (after label and image)
...
class KineticList(evas.ClippedSmartObject):
    ...
    def __on_mouse_clicked(self, edje_obj, emission, source, data=None):
        ...
        elem = self.elements[self.last_start_row + self.objects.index(edje_obj)]
        if elem in self.selection:
            self.selection.remove(elem)
            edje_obj.signal_emit("unselect", "")
        else:
            self.selection.append(elem)
            edje_obj.signal_emit("select", "")
    ...
    def __on_mouse_move(self, edje_obj, emission, source, data=None):
        if self.mouse_down:
            ...
            y1, y2 = self.mouse_down_object_y_range
            if y1 < y_pos < y2:
                return
    ...
    def __on_mouse_down(self, edje_obj, emission, source, data=None):
        ...
        x_pos, y_pos = self.canvas.pointer_canvas_xy
        x, y, w, h = edje_obj.geometry
        self.mouse_down_object_y_range = (y, y + h)
    ...
    def row_add(self, label, *args):
        self.elements.append((label,) + args)
            ...
            for k in range(self.max_horiz_elements):
                ...
                if start_row != self.last_start_row:
                    ...
                    if row in self.selection:
                        self.objects[obj_iter].signal_emit("select","")
                    else:
                        self.objects[obj_iter].signal_emit("unselect","")
